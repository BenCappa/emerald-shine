 ...
 * Task_NewGameBirchSpeech_StartBirchRotomPlatformFade
 * Task_NewGameBirchSpeech_StartBirchRotomPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchRotom
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForRotom
 * Task_NewGameBirchSpeechSub_WaitForRotom
...
static void Task_NewGameBirchSpeechSub_WaitForRotom(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchRotom(u8);
...
static void Task_NewGameBirchSpeech_MainSpeech(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchRotomPlatformFade;
    }
}
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tRotomSpriteId;
    ...
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForRotom;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForRotom(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tRotomSpriteId];
...
static void Task_NewGameBirchSpeech_StartBirchRotomPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        sStartedPokeBallTask = FALSE;
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tRotomSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tRotomSpriteId].invisible = TRUE;
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tRotomSpriteId);
        NewGameBirchSpeech_SetDefaultPlayerName();
        DestroyTask(taskId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchRotom;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchRotom(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tRotomSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tRotomSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tRotomSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateRotomSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_ROTOM, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    rotomSpriteId = NewGameBirchSpeech_CreateRotomSprite(100, 0x4B);
    gSprites[rotomSpriteId].callback = SpriteCB_Null;
    gSprites[rotomSpriteId].oam.priority = 0;
    gSprites[rotomSpriteId].invisible = TRUE;
    gTasks[taskId].tRotomSpriteId = rotomSpriteId;
