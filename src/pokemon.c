...
void CalculateMonStats(struct Pokemon *mon)
{
    ...
    for (u32 i = 0; i < NUM_STATS; i++)
    {
        ...
        s32 n = (((2 * baseStat / 4) * level) / 100) + 5;
    ...
    {
        s32 n = 2 * GetSpeciesBaseHP(species);
        newMaxHP = (((n / 4) * level) / 100) + level + 10;
    }
...
bool32 DoesMonMeetAdditionalConditions(struct Pokemon *mon, const struct EvolutionParam *params, struct Pokemon *tradePartner, u32 partyId, bool32 *canStopEvo, enum EvoState evoState)
{
    ...
    for (i = 0; params != NULL && params[i].condition != CONDITIONS_END; i++)
    {
        ...
        case IF_BAG_ITEM_COUNT:
            ...
            break;
        case IF_MONEY_COUNT:
            if (GetMoney(&gSaveBlock1Ptr->money) >= params[i].arg1)
            {
                currentCondition = TRUE;
                RemoveMoney(&gSaveBlock1Ptr->money, params[i].arg1);
                if (canStopEvo != NULL)
                    *canStopEvo = FALSE;
            }
        case IF_REGION:
        ...
        for (i = 0; formChanges[i].method != FORM_CHANGE_TERMINATOR; i++)
        {
            if (method == formChanges[i].method && species != formChanges[i].targetSpecies)
            {
                ...
                case FORM_CHANGE_TIME_OF_DAY:
                    ...
                    break;
                case FORM_CHANGE_TIME_OF_YEAR:
                    switch (formChanges[i].param1)
                    {
                    case SPRING:
                        if (GetMonth() == MONTH_JAN || GetMonth() == MONTH_MAY || GetMonth() == MONTH_SEP)
                            targetSpecies = formChanges[i].targetSpecies;
                        break;
                    case SUMMER:
                        if (GetMonth() == MONTH_FEB || GetMonth() == MONTH_JUN || GetMonth() == MONTH_OCT)
                            targetSpecies = formChanges[i].targetSpecies;
                        break;
                    case AUTUMN:
                        if (GetMonth() == MONTH_MAR || GetMonth() == MONTH_JUL || GetMonth() == MONTH_NOV)
                            targetSpecies = formChanges[i].targetSpecies;
                        break;
                    case WINTER:
                        if (GetMonth() == MONTH_APR || GetMonth() == MONTH_AUG || GetMonth() == MONTH_DEC)
                            targetSpecies = formChanges[i].targetSpecies;
                        break;
                    }
                default:
